...
const Player: React.FC = () => {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      {videoUrl && (
        <YouTube videoId={extractVideoId(videoUrl)} onReady={handleVideoReady}
                 className={styles.video_player} opts={playerOpts} />
      )}
    </ThemeProvider>
  );
};
...
const extractVideoId = (url: string): string => {
  const match = url.match(/(?:https?:\/\/)?(?:www\.)?youtu(?:\.be|be\.com)\/(?:watch\?v=|embed\/|v\/|shorts\/)?([\w-]{11})/);
